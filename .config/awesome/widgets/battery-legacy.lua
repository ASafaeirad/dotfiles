...
local function substitute(template, context)
    if type(template) ~= "string" then return template(context) end
...
function battery_widget:init(args)
    ...
    self.text_widget.font = args.widget_font
    self.tooltip = awful.tooltip({ objects = { self.text_widget } })

    self.icon_widget = wibox.widget.textbox()
    self.icon_widget.font = (args.icon_widget_font or "Font Awesome 6 Free 7")
...
function battery_widget:update()
    ...
    -- update text
    self.icon_widget:set_markup(substitute("${color_on}${AC_BAT}${color_off}", ctx))
    self.text_widget:set_markup(substitute(self.widget_text, ctx))
